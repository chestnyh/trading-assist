...
const createSortableContainer = (Builder: any, CanMoveFn: any = null): any => 
  class SortableContainer extends Component<SortableContainerProps> {
    tree: any;
    _cacheEls: any;
    eventTarget: any;
    _isUsingLegacyReactDomRender: any;
    ...
    onPropsChanged(nextProps: SortableContainerProps): void {
      this.tree = getFlatTree(nextProps.tree, nextProps.config);
    }

    shouldComponentUpdate(nextProps: SortableContainerProps, nextState: any): boolean {
      ...
      if (should) {
        if (prevState == nextState && prevProps != nextProps) {
          ...
          for (let k in nextProps) {
            let changed = (nextProps[k as keyof SortableContainerProps] != prevProps[k as keyof SortableContainerProps]);
            if (changed) {
    ...
    componentDidUpdate(_prevProps: SortableContainerProps, _prevState: any): void {
    ...
    _getNodeElById (treeEl: any, indexId: any, ignoreCache: boolean = false): any {
      if (indexId == null)
        return null;
      if (!this._cacheEls)
        this._cacheEls = {};
    ...
    _getDraggableNodeEl (treeEl: any, ignoreCache: boolean = false): any {
      if (!this._cacheEls)
        this._cacheEls = {};
    ...
    _getPlaceholderNodeEl (treeEl: any, ignoreCache: boolean = false): any {
      if (!this._cacheEls)
        this._cacheEls = {};
    ...
    _isScrollable(node: any): boolean {
      const overflowY = (window.getComputedStyle(node) as any)["overflow-y"];
      return (overflowY === "scroll" || overflowY === "auto") && (node.scrollHeight > node.offsetHeight);
    }

    _getScrollParent(node: any): any {
      if (node == null)
        return null;
    ...
    _getEventTarget = (e: any, dragStart: any): any => {
      return e && e.__mocked_window || document.body || window;
    };

    onDragStart = (id: any, dom: any, e: any): void => {
    ...
    onDrag = (e: any, doHandleDrag: boolean = true): void => {
    ...
    onDragEnd = (): void => {
      ...
      const target = this.eventTarget || this._getEventTarget(null, null);
      target.removeEventListener("mousemove", this.onDrag);
      target.removeEventListener("mouseup", this.onDragEnd);
    };
    

    handleDrag (dragInfo: any, e: any, canMoveFn: any): boolean {
      ...
      const dragEl = this._getDraggableNodeEl(treeEl);
      const plhEl = this._getPlaceholderNodeEl(treeEl);
      let dragRect: any, plhRect: any, hovRect: any, treeRect: any;
      if (dragEl && plhEl) {
        ...
        if (e.__mocked_hov_container) {
          ...
          if (!hovCNodeEl && hovNodeEl && (hovNodeEl as any).classList.contains("query-builder-container")) {
            // fix 2022-01-24 - get root .group-or-rule-container
            const rootGroupContainer = (hovNodeEl as any)?.firstChild?.firstChild;
        ...
        if (!hovCNodeEl) {
          ...
          if (hovEl) {
            ...
            let trgRect: any = null,
              trgEl: any = null,
              trgII: any = null,
              altII: any = null; //for canMoveBeforeAfterGroup
            ...
            if (trgRect) {
              ...
              let availMoves: any[] = [];
              let altMoves: any[] = []; //alternatively can move after/before group, if can't move into it
              if (isSamePos) {
                //do nothing
              } else {
                if (isGroup) {
              ...
              //sanitize
              availMoves = availMoves.filter((am: any) => {
              ...
              let filteredMoves = availMoves.filter((am: any) => this.canMove(itemInfo, am[1], am[0], am[3], am[4], am[5], canMoveFn));
              if (canMoveBeforeAfterGroup && filteredMoves.length == 0 && altMoves.length > 0) {
                filteredMoves = altMoves.filter((am: any) => this.canMove(itemInfo, am[1], am[0], am[3], am[4], am[5], canMoveFn));
              }
              const levs = filteredMoves.map((am: any) => am[2]);
              const curLev = itemInfo.lev;
              const allLevs = levs.concat(curLev);
              let closestDragLev: any = null;
              ...
              bestMode = filteredMoves.find((am: any) => am[2] == closestDragLev);
    ...
    canMove (fromII: any, toII: any, placement: any, fromCaseII: any, toCaseII: any, toParentII: any, canMoveFn: any): boolean {
      if (!fromII || !toII)
        return false;
    ...
    move (fromII: any, toII: any, placement: any, toParentII: any): void {
...
export default (Builder: any, CanMoveFn: any = null): any => {
