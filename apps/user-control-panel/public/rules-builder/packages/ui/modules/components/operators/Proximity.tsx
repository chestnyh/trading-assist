...
export default class Proximity extends PureComponent<ProximityProps> {
  static propTypes = {
  ...
  render() {
    ...
    const proxValues = range(minProximity || 2, (maxProximity || 10) + 1).map((item: number) => ({title: item, value: item}));
    const factory = widgets.select.factory;
    const Select = (props: any) => factory(props, ctx);
