...
const TRUNCATE_DATETIME = {
  ...
  spelFormatFunc: ({date, dim}: any): any => {
  ...
  // MySQL
  sqlFormatFunc: ({date, dim}: any, sqlDialect: any): any => {
  ...
  mongoFormatFunc: function ({date, dim}: any): any {
    return {
      "$dateTrunc": {
        "date": date,
        "unit": dim,
  ...
  formatFunc: ({date, dim}: any): any => (`TRUNCATE ${date} TO ${dim}`),
  args: {
    date: {
      label: "Datetime",
      type: "datetime",
...
const RELATIVE_DATETIME = {
  ...
  renderBrackets: ["", ""],
  renderSeps: ["", "", ""],
  spelFormatFunc: ({date, op, val, dim}: any): any => {
  ...
  jsonLogic: ({date, op, val, dim}: any): any => ({
    "datetime_add": [
      date,
      val * (op == "minus" ? -1 : +1),
      dim
  ...
  sqlFormatFunc: ({date, op, val, dim}: any): any => `DATE_ADD(${date}, INTERVAL ${parseInt(val) * (op == "minus" ? -1 : +1)} ${dim.replace(/^'|'$/g, "")})`,
  sqlImport: function (sqlObj: any, _: any, sqlDialect: any): any {
  ...
  mongoFormatFunc: function ({date, op, val, dim}: any): any {
    return {
      "$dateAdd": {
        "startDate": date,
        "unit": dim,
  ...
  formatFunc: ({date, op, val, dim}: any): any => (!val ? date : `${date} ${op == "minus" ? "-" : "+"} ${val} ${dim}`),
  args: {
    date: {
      label: "Datetime",
      type: "datetime",
...
const LINEAR_REGRESSION = {
  ...
  returnType: "number",
  formatFunc: ({coef, bias, val}: any, _: any): any => `(${coef} * ${val} + ${bias})`,
  sqlFormatFunc: ({coef, bias, val}: any): any => `(${coef} * ${val} + ${bias})`,
  spelFormatFunc: ({coef, bias, val}: any): any => `(${coef} * ${val} + ${bias})`,
  spelImport: (spel: any): any => {
    let coef, val, bias, a;
  ...
  mongoFormatFunc: ({coef, bias, val}: any): any => ({"$sum": [{"$multiply": [coef, val]}, bias]}),
  jsonLogic: ({coef, bias, val}: any): any => ({ "+": [ {"*": [coef, val]}, bias ] }),
  jsonLogicImport: (v: any): any => {
    const coef = v["+"][0]["*"][0];
    const val = v["+"][0]["*"][1];
    const bias = v["+"][1];
