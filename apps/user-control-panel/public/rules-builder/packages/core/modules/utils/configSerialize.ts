...
function applyJsonLogicWithPath(logic: any, data: any, path: string[]) {
  let ret;
  try {
...
function callContextFn(_this: any, fn: any, args: any, path: string[]) {
  let ret;
  try {
...
export const compressConfig = (config: any, baseConfig: any) => {
  if (config.__fieldNames) {
    throw new Error("Don't apply `compressConfig()` to extended config");
  }
  let zipConfig: any = pick(config, configKeys);
  delete zipConfig.ctx;

  const _clean = (target: any, base: any, path: any[], meta: any): any => {
    if (isObject(target)) {
...
export const decompressConfig = (zipConfig: any, baseConfig: any, ctx: any) => {
  ...
  const _mergeDeep = (target: any, mixin: any, path: any[]): any => {
    if (isObject(mixin)) {
      if (!isObject(target)) {
        target = {};
      }
  ...
  const _resolveAndMergeDeep = (target: any, path: any[], meta: any): any => {
...
export const compileConfig = (config: any) => {
...
function _compileConfigParts(config: any, subconfig: any, opts: any, meta: any, logs: string[], path: string[] = []) {
  if (!subconfig) return;
...
function compileJsonLogicReact(jl: any, opts: any, path: string[], ignore: string | undefined = undefined): any {
  if (isJsonLogic(jl)) {
    return function(props: any, ctx: any) {
    ...
    return function(props: any, ctx: any) {
      ctx = ctx || opts?.ctx; // can use context compile-time if not passed at runtime
      const fn = jl.split(".").reduce((o: any, k: string) => o?.[k], ctx);
      if (fn) {
        return callContextFn(opts?.ctx, fn, [props, ctx], path);
      } else {
        const re = {
          type: jl,
          props
...
function compileJsonLogic(jl: any, opts: any, path: string[], argNames: string[] | undefined, ignore: string | undefined = undefined): any {
  if (isJsonLogic(jl) && ignore !== "jl") {
    return function(...args: any[]) {
      const ctx = opts?.ctx; // can use context compile-time if not passed at runtime
      const data = (argNames || []).reduce((acc: any, k: string, i: number) => ({...acc, [k]: args[i]}), {
        args, ctx
      });
    ...
    return function(...args: any[]) {
      ...
      const fn = jl.split(".").reduce((o: any, k: string) => o?.[k], ctx);
      if (fn) {
        return callContextFn(opts?.ctx, fn, args, path);
      } else {
...
function getReactComponentFromCtx(name: string, ctx: any): any {
  return ctx?.components?.[name] || ctx?.W?.[name] || ctx?.O?.[name];
}

function renderReactElement(jsx: any, opts: any, path: string[], key: string | undefined = undefined): any {
  if (isJSX(jsx)) {
    let { type, props } = jsx;
    ...
    return jsx.map((el: any, i: number) => renderReactElement(el, opts, path, String(i)));
  }
  return jsx;
}
