...
export const isValidTree = (tree: any, config: any) => {
...
export const getTreeBadFields = (tree: any, config: any): string[] => {
  ...
  function _processNode (item: any, path: any[], lev: number): void {
    ...
    const hasValueError = valueError?.size > 0 && valueError.filter((v: any) => v != null).size > 0;
...
// @deprecated
export const checkTree = (tree: any, config: any) => {
...
*/
export const validateTree = (tree: any, config: any, options = {}) => {
  const extendedConfig = extendConfig(config, undefined, true);
  const finalOptions = {
    ...options,
 ...
 * }}
 */
export const sanitizeTree = (tree: any, config: any, options = {}) => {
  const extendedConfig = extendConfig(config, undefined, true);
  const finalOptions = {
    // defaults
...
export const validateAndFixTree = (newTree: any, _oldTree: any, newConfig: any, oldConfig: any, removeEmptyGroups?: any, removeEmptyRules?: any, removeIncompleteRules?: any) => {
...
export const _validateTree = (
  tree: any, _oldTree: any, config: any, oldConfig: any, options: any
) => {
  if (!tree) {
    return {
      fixedTree: tree,
  ...
  const meta: { errors: Record<string, { path: string[]; errors: any[] }>; sanitized: boolean } = {
    errors: {},
    sanitized: false,
  };
    ...
    if (includeItemsPositions) {
      ...
      if (flatItem) {
        ...
        if (stringifyItemsUserFriendly) {
          ...
          trArgs.indexPath = itemPosition.indexPath?.map((ind: any) => ind+1);
        }
  ...
  let fixedErrors: any[] = [];
  let nonFixedErrors: any[] = [];
  for (const itemErrors of allErrors) {
    const fixedItemErrors = itemErrors.errors.filter((e: any) => !!e.fixed);
    let nonFixedItemErrors = itemErrors.errors.filter((e: any) => !e.fixed && e.key !== "EMPTY_QUERY");
...
function _addError(meta: any, item: any, path: any, err: any) {
  const id = item.get("id");
  if (!meta.errors[id]) {
    meta.errors[id] = {
      path: [...path, id],
...
function _setErrorsAsFixed(meta: any, item: any) {
  const id = item.get("id");
  if (meta.errors[id]) {
    meta.errors[id].errors.map((e: any) => {
      e.fixed = true;
    });
  }
}

function validateItem (item: any, path: any, itemId: any, meta: any, c: any) {
...
function validateGroup (item: any, path: any, itemId: any, meta: any, c: any) {
  ...
  children = children
    ?.map( (currentChild: any, childId: any) => validateItem(currentChild, path.concat(id), childId, submeta, c) );
  const nonEmptyChildren = children?.filter((currentChild: any) => (currentChild != undefined));
 ...
 */
function validateRule (item: any, path: any, itemId: any, meta: any, c: any) {
  ...
  if (!properties) {
    if (isCase) {
      properties = new (Immutable.Map as any)();
    } else {
      const err = {
  ...
  const compl: any = whatRulePropertiesAreCompleted(properties.toObject(), config);
  ...
  if (!isCompleted) {
    if (isCase) {
      ...
      //   }
      // }
      let incError: any = { key: constants.INCOMPLETE_RULE, args: {} };
 ...
 */
export const validateValue: any = (
  config: any, leftField: any, field: any, operator: any, value: any, valueType: any, valueSrc: any, asyncListValues: any,
  ...
  if (value != null) {
    ...
    let fixedAllErrors = !allErrors?.find((e: any) => !e.fixed);
    ...
    fixedAllErrors = !allErrors?.find((e: any) => !e.fixed);
 ...
 * 
 */
const validateValueInList = (value: any, listValues: any, canFix: any, isEndValue: any, removeInvalidMultiSelectValuesOnLoad: any) => {
 ...
 * 
 */
const validateNormalValue = (field: any, value: any, valueSrc: any, valueType: any, asyncListValues: any, config: any, operator = null, canFix = false, isEndValue = false) => {
  let allErrors = [];
  let fixedValue = value;
  if (field) {
    const fieldConfig = getFieldConfig(config, field);
    const w = getWidgetForFieldOp(config, field, operator, valueSrc);
    const wConfig = w ? config.widgets[w] : undefined;
 ...
 * 
 */
const validateFieldValue = (leftField: any, field: any, value: any, _valueSrc: any, valueType: any, asyncListValues: any, config: any, operator = null, canFix = false, isEndValue = false) => {
  const allErrors = [];
 ...
 */
const validateFuncValue: any = (
  leftField: any, field: any, value: any, _valueSrc: any, valueType: any, asyncListValues: any, config: any, operator = null,
  canFix = false, isEndValue = false, canDropArgs = false
) => {
  let fixedValue = value;
  const allErrors = [];
  ...
  for (const argKey in funcConfig.args) {
    ...
    if (argValue !== undefined) {
      const [fixedArgVal, argErrors]: any = validateValue(
      ...
      if (!isValid) {
        const firstError = argErrors.find((e: any) => !e.fixed && !e.ignore) ?? argErrors.find((e: any) => !e.fixed) ?? argErrors[0];
        const fixed = willFix || canDropOrReset;
        const ignore = argErrors.filter((e: any) => !e.ignore).length === 0;
        if (firstError) {
 ...
 * 
 */
export const validateRange = (config: any, field: any, operator: any, values: any, valueSrcs: any) => {
  ...
  const areValueSrcsPureValues = valueSrcsArr.filter((vs: any) => vs == "value" || vs == null).length == operatorCardinality;
  let rangeError;
