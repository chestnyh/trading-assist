 ...
 */
export const setFunc = (value: any, funcKey: any, config: any, canFixArgs: any): any => {
  ...
  if (keepArgsKeys.length) {
    ...
    value = deleteArgsKeys.reduce((value: any, k: any) => value.deleteIn(["args", k]), value);
  } else {
...
*/
export const setArgValueSrc = (value: any, argKey: any, argValSrc: any, _argConfig: any, _config: any): any => {
...
export const setArgValueAsyncListValues = (value: any, argKey: any, asyncListValues: any, _argConfig: any, _config: any): any => {
  if (value && value.get("func")) {
...
export const getCompatibleArgsOnFuncChange = (s1: any, s2: any, argVals: any, config: any, canFixArgs = false): any[] => {
  ...
  const compatibleKeys = keys.filter((k, i) => {
    ...
    if (argVals) {
      ...
      if (argValidErrors?.filter((e: any) => !e.fixed)?.length)
        return false;
    }
