...
const getReactRootNodeType = (node: any) => {
  if (!node) {
    return undefined;
  }
...
export const isUsingLegacyReactDomRender = (node: any) => {
  return getReactRootNodeType(node) === "root";
};


export const liteShouldComponentUpdate = (self: any, config: any) => (nextProps: any, nextState: any) => {
...
export const pureShouldComponentUpdate = (self: any) => function(nextProps: any, nextState: any) {
  return (
...
const canUseOldComponentWillReceiveProps = () => {
  const v = React.version.split(".").map((v: any) => parseInt(v, 10));
  return v[0] == 16 && v[1] < 3 || v[0] < 16;
};

export const useOnPropsChanged = (obj: any) => {
  ...
  if (canUseOldComponentWillReceiveProps()) {
    // Use old method
    obj.componentWillReceiveProps = (nextProps: any) => {
    ...
    const newShouldComponentUpdate = function(nextProps: any, nextState: any) {
...
export const bindActionCreators = (actionCreators: any, config: any, dispatch: any) =>
  mapValues(actionCreators, (actionCreator: any) =>
    (...args: any[]) => dispatch(actionCreator(config, ...args))
  );
