...
const getNumberFormatProps = (props: any, excludePropsNames: string[] = []) => {
  const propNames = [
    // Numeric format
    "prefix",
    "suffix",
...
const numericFormatter = (val: any, numericFormatProps: any) => {
  if (val == undefined) {
    return "";
  }
...
function getDefaultChangeMeta(value: any) {
  return {
    from: {
      start: 0,
      end: 0,
...
const findChangeRange = (prevValue: any, newValue: any) => {
  let i = 0,
    j = 0;
...
const getChangeMeta = (newValue: any, prevValue: string | undefined = undefined) => {
...
// Apply `decimalScale`
const limitToScale = (num: any, numericFormatProps: any) => {
  ...
  let decimalPart: number | undefined = decimalStr != undefined ? parseFloat("0." + decimalStr) : undefined;
  ...
  const decimalValue = decimalPart ?? 0;
  const res = (isNegative ? -1 : 1) * (positiveWholeNumberPart + decimalValue); // -3.14
  return res;
};

const startsWithZero = (num: any) => {
  const isNegative = num < 0;
  const positiveWholeNumberPart = Math.abs(isNegative ? Math.ceil(num) : Math.floor(num));
  return positiveWholeNumberPart === 0;
};

const getExtraLeadingZeros = (str: any, num: any) => {
...
const getTrailingZeros = (str: any) => {
...
const fixDecimal = (origStr: any, numericFormatProps: any, lastStrValue: string | undefined = undefined) => {
  const decimalSeparator = numericFormatProps?.decimalSeparator ?? ".";
  let str = origStr;
  if (lastStrValue && lastStrValue.endsWith(".") && origStr.indexOf(lastStrValue) === 0) {
...
const numericParser = (origStr: any, numericFormatProps: any, lastStrValue: string | undefined = undefined, lastNumValue: number | undefined = undefined) => {
  const decimalSeparator = numericFormatProps?.decimalSeparator ?? ".";
  ...
  let num: number | undefined = parseFloat(cleanStr);
...
const addLeadingZeros = (str: any, extraLeadingZeros: any) => {
...
const addTrailingZeros = (str: any, trailingZeros: any) => {
