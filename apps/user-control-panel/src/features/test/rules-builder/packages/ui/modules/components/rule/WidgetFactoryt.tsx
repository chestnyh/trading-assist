...
const WidgetFactory = ({
  ...
  const fixedImmValue = immValue ? immValue.map((v: any) => _fixImmutableValue(v)) : undefined;
  const value = useMemo(() => {
    if (isSpecialRange) {
      let value: any[] | undefined = [ fixedImmValue?.get(0), fixedImmValue?.get(1) ];
