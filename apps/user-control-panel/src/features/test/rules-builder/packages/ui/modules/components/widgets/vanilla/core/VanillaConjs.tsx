...
export default ({id, not, setNot, conjunctionOptions, setConjunction, disabled, readonly, config, showNot, notLabel}: {id: string, not: boolean, setNot: (not: boolean) => void, conjunctionOptions: any, setConjunction: ((conjunction: string) => void) & {isDummyFn?: boolean}, disabled: boolean, readonly: boolean, config: any, showNot: boolean, notLabel: string}) => {
  ...
  const onChange = (e: React.ChangeEvent<HTMLInputElement>) => setConjunction(e.target.value);

  const onNotChange = (e: React.ChangeEvent<HTMLInputElement>) => setNot(e.target.checked);
