...
const useListValuesAutocomplete = ({
  ...
  // fetch - search
  const fetchListValues = async (filter: string | null | undefined = null, isLoadMore: boolean = false) => {
    // clear obsolete meta
  ...
  const fetchSelectedListValues = async () => {
    ...
    const nValues = latestSelectedValues.map((v: any) => getItemInListValues(selectedListValues, v) ?? makeCustomListValue(v));
  ...
  const loadListValues = async (filter: string | null | undefined = null, isLoadMore: boolean = false) => {
    setLoadingCnt(x => (x + 1));
  ...
  const onChange = async (e: any, val: any, option: any) => {
    ...
    if (uif === "mui") {
      option = val;
      if (multiple) {
        val = option.map((o: any) => (o?.value ?? o));
      } else {
    ...
    if (specialValue == "LOAD_MORE") {
      ...
      await loadListValues(inputValue || undefined, true);
  ...
  const onInputChange = async (e: any, newInputValue: string, eventType: string) => {
    ...
    if (newInputValue != inputValue) {
      ...
      if (canSearchAsync) {
        await loadListValuesDebounced(val || undefined);
  ...
  const getOptionSelected = (option: any, valueOrOption: any) => {
  ...
  const getOptionDisabled = (valueOrOption: any) => {
    return valueOrOption && valueOrOption.disabled;
  };

  const getOptionIsCustom = (valueOrOption: any) => {
  ...
  const getOptionLabel = (valueOrOption: any) => {
    if (valueOrOption == null)
      return null;
