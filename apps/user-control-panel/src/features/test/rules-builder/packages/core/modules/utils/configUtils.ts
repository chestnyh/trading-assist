...
export function* iterateFuncs(config: any) {
  yield* _iterateFields(config, config.funcs || {}, []);
}

export function* iterateFields(config: any) {
  yield* _iterateFields(config, config.fields || {}, []);
}

function* _iterateFields(config: any, subfields: any, path: string[], subfieldsKey = "subfields"): any {
...
export const getFieldRawConfig = (config: any, field: any, fieldsKey = "fields", subfieldsKey = "subfields"): any => {
  if (!field)
    return null;
  if (field === "!case_value") {
...
export const normalizeField = (config: any, field: any, parentField: string | null = null): string => {
  ...
  const findStr = field;
  const normalizedPath = config.__fieldNames[findStr]?.find?.(({inGroup}: {inGroup: string}) => {
    if (inGroup)
...
export const getFuncSignature = (config: any, func: any): any => {
  if (!func)
    return null;
...
export const getFuncConfig = (config: any, func: any): any => {
  if (!func)
    return null;
  const funcConfig = getFieldRawConfig(config, func, "funcs", "subfields");
  if (!funcConfig)
...
export const getFuncArgConfig = (config: any, funcKey: any, argKey: any): any => {
...
export const isFieldDescendantOfField = (field: any, parentField: any, config: any = null): boolean => {
...
export const getFieldPath = (field: any, config: any = null): string => {
...
export const getFieldParts = (field: any, config: any = null): string[] => {
  if (!field)
    return [];
...
export const getFieldPathParts = (field: any, config: any, onlyKeys = false): string[] | null => {
  if (!field)
    return null;
  const fieldSeparator = config.settings.fieldSeparator;
  const parts = getFieldParts(field, config);
...
export const getFieldId = (field: any): string | null => {
...
export const _getFromConfigCache = (config: any, bucketKey: string, cacheKey: string): any => {
  return config.__cache?.[bucketKey]?.[cacheKey];
};

export const _saveToConfigCache = (config: any, bucketKey: string, cacheKey: string, value: any): void => {
...
export const getFieldSrc = (field: any): string | null => {
  if (!field)
    return null;
...
export const getFieldConfig = (config: any, field: any): any => {
  if (!field)
    return null;
  if (typeof field == "object") {
...
export const getOperatorConfig = (config: any, operator: any, field: any = null): any => {
  if (!operator)
    return null;
  ...
  if (field) {
    ...
    const cached = _getFromConfigCache(config, "getOperatorConfig", cacheKey as string);
    if (cached)
      return cached;
    const fieldConfig = getFieldConfig(config, field);
    const widget = getWidgetForFieldOp(config, field, operator, null);
    const widgetConfig = config.widgets[widget as string] || {};
    const fieldWidgetConfig = (fieldConfig && fieldConfig.widgets ? fieldConfig.widgets[widget as string] : {}) || {};
    ...
    _saveToConfigCache(config, "getOperatorConfig", cacheKey as string, mergedConfig);
    return mergedConfig;
  } else {
    return opConfig;
  }
...
export const getFieldWidgetConfig = (config: any, field: any, operator = null, widget: string | null = null, valueSrc = null, meta: any = {}) => {
  if (!field)
    return null;
  const fieldConfig = getFieldConfig(config, field);
  const fieldCacheKey = getFieldId(field);
  if (!widget) {
    widget = getWidgetForFieldOp(config, field, operator, valueSrc) as string | null;
  }
  if (!widget) {
    return null;
  }
...
export const getFirstField = (config: any, parentRuleGroupField: any = null): string | null => {
  const fieldSeparator = config.settings.fieldSeparator;
...
export function _getWidgetsAndSrcsForFieldOp (config: any, field: any, operator = null, valueSrc = null): {widgets: string[], valueSrcs: string[]} {
  let widgets: string[] = [];
  let valueSrcs: string[] = [];
  if (!field)
  ...
  const widgetWeight = (w: string): number => {
    let wg = 0;
...
export const filterValueSourcesForField = (config: any, valueSrcs: string[], fieldDefinition: any, operator = null): string[] => {
  if (!fieldDefinition)
    return valueSrcs;
...
export const getWidgetForFieldOp = (config: any, field: any, operator: any, valueSrc = null): string | null => {
...
export const getValueSourcesForFieldOp = (config: any, field: any, operator: any, fieldDefinition = null): string[] => {
...
export const getWidgetsForFieldOp = (config: any, field: any, operator: any, valueSrc = null): string[] => {
  const {widgets} = _getWidgetsAndSrcsForFieldOp(config, field, operator, valueSrc);
  return widgets;
};

export const getOperatorsForType = (config: any, fieldType: string): string[] | null => {
  return config.types[fieldType]?.operators || null;
};

export const getOperatorsForField = (config: any, field: any): string[] | null => {
  const fieldConfig = getFieldConfig(config, field);
  const fieldOps = fieldConfig ? fieldConfig.operators : [];
  return fieldOps;
};

export const getFirstOperator = (config: any, field: any): string | null => {
  const fieldOps = getOperatorsForField(config, field);
  return fieldOps?.[0] ?? null;
};

export const getFieldPartsConfigs = (field: any, config: any, parentField: any = null): any[] | null => {
  if (!field)
    return null;
