...
export const jsonLogicFormat = (item: any, config: any): any => {
  //meta is mutable
  let meta = {
    usedFields: [] as any[],
    errors: [] as any[]
  ...
  for (let ff of usedFields) {
    ...
    let tmp: Record<string, any> = data;
...
const formatItem = (item: any, config: any, meta: any, _not = false, isRoot = false, parentField: any = null): any => {
  if (!item) return undefined;
  const type = item.get("type");
  const properties = item.get("properties") || new (Map as any)();
...
const formatGroup = (item: any, config: any, meta: any, _not = false, isRoot = false, parentField: any = null): any => {
  const type = item.get("type");
  const properties = item.get("properties") || new (Map as any)();
  const mode = properties.get("mode");
  const children = item.get("children1") || new (List as any)();
  ...
  const list = children
    .map((currentChild: any) => formatItem(currentChild, config, meta, revChildren, false, groupField))
  ...
  if (!preserveSingleRuleGroup && origNot && !revChildren) {
    ...
    preserveSingleRuleGroup = children.some((currentChild: any) => {
  ...
  if (isRuleGroupArray) {
    ...
    const optionsMap = new (Map as any)({
      having: filterQuery,
      reduce: reduceQuery,
      groupField: field,
...
const formatRule = (item: any, config: any, meta: any, _not = false, parentField: any = null): any => {
  const properties = item.get("properties") || new (Map as any)();
  const field = properties.get("field");
...
const formatSwitch = (item: any, config: any, meta: any, _not = false): any => {
  const children = item.get("children1");
  if (!children)
    return undefined;
  const cases = children
    .map((currentChild: any) => formatCase(currentChild, config, meta, _not, null))
    .filter((currentChild: any) => typeof currentChild !== "undefined")
    .valueSeq().toArray();
  ...
  for (let i = 0 ; i < filteredCases.length - 1 ; i++) {
    ...
    if (isLastIf) {
      ...
      // elif..
      ifArgs.push({ if: [] });
      ifArgs = (ifArgs[ifArgs.length - 1] as any).if;
    }
  }
  return ret;
};

const formatCase = (item: any, config: any, meta: any, _not = false, parentField: any = null): any => {
  const type = item.get("type");
  if (type != "case_group") {
  ...
  const cond = formatGroup(item, config, meta, _not, false as any, parentField);

  const formattedItem = formatItemValue(
    config, properties, meta, null, parentField, "!case_value" as any
  );
  return [cond, formattedItem];
};

const formatItemValue = (config: any, properties: any, meta: any, operator: any, parentField: any, expectedValueType: any = null): any => {
  ...
  const fvalue = iValue.map((currentValue: any, ind: any) => {
  ...
  const hasUndefinedValues = fvalue.filter((v: any) => v === undefined).size > 0;
...
const formatValue = (meta: any, config: any, currentValue: any, valueSrc: any, valueType: any, fieldWidgetDef: any, fieldDef: any, operator: any, operatorDef: any, parentField: any = null, asyncListValues: any): any => {
...
const formatFunc = (meta: any, config: any, currentValue: any, parentField: any = null): any => {
  ...
  let formattedArgs: Record<string, any> = {};
  let gaps: string[] = [];
  let missingArgKeys: string[] = [];
  for (const argKey in funcConfig.args) {
    ...
    const operator = null;
    const widget = getWidgetForFieldOp(config, argConfig, operator, argValueSrc as any);
    const fieldWidgetDef = getFieldWidgetConfig(config, argConfig, operator, widget, argValueSrc as any, { forExport: true });
    const formattedArgVal = formatValue(
      meta, config, argValue, argValueSrc, argConfig.type, fieldWidgetDef, fieldDef, null, null, parentField, undefined
    );
    ...
    let formattedDefaultVal: any;
    if (formattedArgVal === undefined && !isOptional && defaultValue != undefined) {
      const defaultWidget = getWidgetForFieldOp(config, argConfig, operator, defaultValueSrc as any);
      const defaultFieldWidgetDef = getFieldWidgetConfig(config, argConfig, operator, defaultWidget, defaultValueSrc as any, { forExport: true });
      formattedDefaultVal = formatValue(
        meta, config, defaultValue, defaultValueSrc as any, argConfig.type, defaultFieldWidgetDef, fieldDef, null, null, parentField, undefined
      );
...
const formatField = (meta: any, config: any, field: any, parentField: any = null): any => {
...
const buildFnToFormatOp = (operator: any, operatorDefinition: any, formattedField: any, formattedValue: any, expectedType: any): any => {
  ...
  const rangeOps = ["<", "<=", ">", ">="];
  const eqOps = ["==", "!="];
  const fn = (field: any, op: any, val: any, opDef: any, opOpts: any) => {
...
const formatLogic = (config: any, properties: any, formattedField: any, formattedValue: any, operator: any, operatorOptions: any = null, fieldDefinition: any = null, isRev = false): any => {
  const field = properties.get("field");
  //const fieldSrc = properties.get("fieldSrc");
  const firstValueType = properties.get("valueType")?.get(0);
  const operatorDefinition = getOperatorConfig(config, operator, field) || {};
  const expectedType = (fieldDefinition as any)?.type ?? firstValueType;
