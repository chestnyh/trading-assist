...
export const extendConfig = (config: any, configId?: string, canCompile = true): any => {
...
function extendTypesConfig(typesConfig: Record<string, any>, config: any): void {
...
function extendTypeConfig(type: string, typeConfig: any, config: any): void {
  let operators: string[] | null = null, defaultOperator: string | null = null;
  typeConfig.mainWidget = typeConfig.mainWidget || Object.keys(typeConfig.widgets).filter((w: string) => w != "field" && w != "func")[0];
  ...
  for (const widget in typeConfig.widgets) {
    ...
    if (typeWidgetConfig.operators) {
      const typeWidgetOperators = typeWidgetConfig.operators.filter((op: string) => !excludeOperators.includes(op));
...
function extendFieldsConfig(subconfig: Record<string, any>, config: any, path: string[] = []): void {
...
function extendFuncsConfig(subconfig: Record<string, any> | undefined, config: any, path: string[] = []): void {
  if (!subconfig) return;
...
function normalizeFieldSettings(fieldConfig: any, config: any, type: string): void {
...
function extendFieldConfig(fieldConfig: any, config: any, path: string[] = [], isFuncArg = false, isCaseValue = false): void {
  ...
  let operators = (fieldConfig.operators || typeConfig?.operators || []).filter((op: string) => !excludeOperatorsForField.includes(op));
  ...
  if (fieldConfig.operators) {
    ...
    excludeOperatorsForType = excludeOperatorsForType.filter((op: string) => !fieldConfig.operators.includes(op));
  }
  ...
  for (const widget in typeConfig.widgets) {
    ...
    if (!isFuncArg) { // tip: operators are not used for func args
      ...
      if (fieldWidgetConfig.operators) {
        const addOperators = fieldWidgetConfig.operators.filter((op: string) => !excludeOperators.includes(op));
        ...
        const addOperators = typeWidgetConfig.operators.filter((op: string) => !excludeOperators.includes(op));
        fieldWidgetConfig.operators = addOperators;
        // operators = [...(operators || []), ...addOperators];
        operators = mergeArraysSmart(operators, addOperators);
      }
    ...
    if (opKeys.length) {
      const opProps: Record<string, any> = {};
...
function computeFieldName(config: any, path: string[]): { fieldName?: string; inGroup?: string } {
  if (!path)
    return {};
  const fieldSeparator = config.settings.fieldSeparator;
  const {computedPath, computed, inGroup} = [...path].reduce((acc: {computedPath: string[]; computed: boolean; inGroup: string | undefined}, f: string, i: number, arr: string[]): {computedPath: string[]; computed: boolean; inGroup: string | undefined} => {
    ...
    if (fConfig?.type === "!group" && i < arr.length-1) {
      ...
      acc.inGroup = fullPath.join(fieldSeparator);
      acc.computedPath = [];
    } else if (fConfig?.fieldName) {
      // tip: fieldName overrides path !
      acc.computed = true;
      acc.computedPath = [fConfig.fieldName];
    } else {
      acc.computedPath = [...acc.computedPath, f];
    }
    return acc;
