...
export const isObjectOrArray = (v: any): boolean => (typeof v === "object" && v !== null);
...
export const isTypeOf = (v: any, type: string): boolean => {
  if (typeOf(v) === type)
    return true;
...
export const shallowCopy = (v: any): any => {
...
export const isSafeKeyForObject = (k: any): boolean => {
  return !["__proto__", "constructor", "prototype"].includes(k);
};

export const hasSafeOwnProperty = (obj: any, k: any): boolean => {
  return Object.prototype.hasOwnProperty.call(obj, k) && isSafeKeyForObject(k);
};

export const setIn = (obj: any, path: any[], newValue: any, opts?: any): any => {
  const defaultOpts = {
...
export const mergeIn = (obj: any, mixin: any, opts?: any): any => {
  const defaultOpts = {
  ...
  const newObj = deepCopyObj ? clone(obj) : shallowCopy(obj);
  let newObjChanged = false;
  const _process = (path: any[], targetMix: any, target: any, {
    isMixingArray = false, isMixingRealArray = false,
  }: { isMixingArray?: boolean; isMixingRealArray?: boolean } = {}) => {
    let indexDelta = 0;
    for (const mk in targetMix) {
      ...
      if (expectedType === "array" || expectedType === "object") {
        if (isMixValueExplicit) {
          ...
          newObjChanged = true;
          target[k] = deepCopyMixin ? clone(targetMixValue) : shallowCopy(targetMixValue);
        } else {
          if (arrayMergeMode && ["join", "joinMissing", "joinRespectOrder"].includes(arrayMergeMode)) {
            // join 2 arrays
            newObjChanged = true;
            const left = (deepCopyObj ? target[k] : clone(target[k]));
            let right = (deepCopyMixin ? clone(targetMixValue) : targetMixValue);
            if (arrayMergeMode === "joinRespectOrder") {
              target[k] = mergeArraysSmart(left, right);
            } else {
              if (arrayMergeMode === "joinMissing") {
                right = right.filter((v: any) => !left.includes(v));
              }
              target[k] = [ ...left, ...right ];
            }
          } else {
...
export const omit = (obj: any, keys: any[]): any => {
...
export const getOpCardinality = (opDef: any): number => {
...
export const deepEqual = function(v1: any, v2: any): boolean {
  if (v1 === v2) {
    return true;
...
export const shallowEqual = (a: any, b: any, deep = false): boolean => {
  if (a === b) {
    return true;
...
function shallowEqualArrays(arrA: any, arrB: any, deep = false): boolean {
  if (arrA === arrB) {
    return true;
  }
...
function shallowEqualObjects(objA: any, objB: any, deep = false): boolean {
  if (objA === objB) {
    return true;
  }
...
export const mergeCustomizerNoArrays = (objValue: any, srcValue: any, _key: any, _object: any, _source: any, _stack: any): any => {
  if (Array.isArray(objValue)) {
    return srcValue;
  }
};

export const cleanJSX = (jsx: any): any => {
  const jsxKeys = ["$$typeof", "_owner", "_store", "ref", "key"];

  const getName = (val: any): any => {
  ...
  if (jsx instanceof Array) {
    return jsx.map((el: any, _i: any) => cleanJSX(el));
  } else if (typeof jsx === "object" && jsx !== null) {
    if (isDirtyJSX(jsx)) {
      const cleaned: any = omit(jsx, jsxKeys);
      if (cleaned.type) {
...
export const isDirtyJSX = (jsx: any): boolean => {
...
export const isJSX = (jsx: any): boolean => (
  typeof jsx === "object"
  && jsx !== null
  && !Array.isArray(jsx)
...
export const isJsonLogic = (logic: any): boolean => {
...
export function mergeArraysSmart(arr1: any[], arr2: any[]): any[] {
  ...
  if (!arr2) arr2 = [];
  const mapped = arr2
    .map((op: any): [any, number] => [op, arr1.indexOf(op)])
    .map((item: [any, number], i: number, orig: [any, number][]) => {
      const [op, ind] = item;
      if (ind == -1) {
        const next = orig.slice(i+1);
        const prevs = orig.slice(0, i);
        const after = [...prevs].reverse().find(([_cop, ci]: [any, number]) => ci != -1);
        const prev = prevs[prevs.length - 1];
        const before = next.find(([_cop, ci]: [any, number]) => ci != -1);
        const isAfterDirectly = after && after === prevs[prevs.length-1];
        const isBeforeDirectly = before && next === next[0];
        if (isAfterDirectly) {
          return [op, "after", after[0]] as [any, string, any];
        } else if (isBeforeDirectly) {
          return [op, "before", before[0]] as [any, string, any];
        } else if (after) {
          if (prev) {
            return [op, "after", prev[0]] as [any, string, any];
          }
          return [op, "after", after[0]] as [any, string, any];
        } else if (before) {
          return [op, "before", before[0]] as [any, string, any];
        } else {
          return [op, "append", null] as [any, string, any];
        }
      } else {
        // already exists
        return null;
    ...
    .filter((x: any): x is [any, string, any] => x !== null);
  return mapped.reduce((acc: any[], item: [any, string, any]) => {
      const [newOp, rel, relOp] = item;
...
export const isJsonCompatible = (tpl: any, obj: any, bag: Record<string, any> = {}, path: any[] = []): boolean => {
  if (isObject(tpl)) {
    if (tpl.var) {
      bag[tpl.var] = obj;
      return true;
...
export const getLogger = (devMode?: boolean): any => {
  if (isTest()) {
    return {
      ...console,
      log: () => {},
...
export const getFirstDefined = (arr: any[] = []): any => {
  let ret;
...
export const logger = getLogger(undefined);
