...
export const getNewValueForFieldOp = function (
  ...
  config: any, oldConfig: any = null, current: any, newField: any, newOperator: any, changedProp: string | null = null,
  ...
  const currentValueError = current.get("valueError", Immutable.List());
  ...
  if (currentFieldSrc === "func" && newField) {
    ...
    const willFixAllErrors = !isValid && willFix && !fieldErrors.find((e: any) => !e.fixed);
    ...
    if (!isValid) {
      ...
      const firstError = fieldErrors.find((e: any) => !e.fixed && !e.ignore);
      ...
      fieldErrors.forEach((e: any) => validationErrors.push({
        side: "lhs",
        ...e,
  ...
  for (let i = 0 ; i < (operatorCardinality || 0) ; i++) {
    ...
    const currentValueWidget = vs === "value" ? currentWidget
      : getWidgetForFieldOp(oldConfig, currentField, currentOperator, "value" as any);
    const newValueWidget = vs === "value" ? newWidget
      : getWidgetForFieldOp(config, newField, newOperator, "value" as any);
  ...
  let valueFixes: any[] = [];
  let valueSrcFixes: any[] = [];
  let valueTypeFixes: any[] = [];
  let valueErrors = Array.from({length: operatorCardinality || 0}, () => null);
  if (canReuseValue) {
    for (let i = 0 ; i < (operatorCardinality || 0) ; i++) {
      ...
      const willFixAllErrors = !isValid && willFix && !allErrors?.find((e: any) => !e.fixed);
      const allErrorsHandled = !allErrors?.find((e: any) => !e.fixed && !e.ignore);
      ...
      if (!isValid) {
        ...
        allErrors?.forEach((e: any) => validationErrors.push({
          side: "rhs",
          delta: i,
          ...e,
  ...
  for (let i = 0 ; i < (operatorCardinality || 0) ; i++) {
    ...
    const v: any = valueFixes[i] !== undefined ? valueFixes[i] : (canReuseValue ? currentValue.get(i) : undefined);
  ...
  if (valueFixes.length > 0 || !canReuseValue || (operatorCardinality || 0) < (currentOperatorCardinality || 0)) {
    newValue = Immutable.List(Array.from({length: operatorCardinality || 0}, (_ignore: any, i: number) => {
      return valueFixes[i] !== undefined ? valueFixes[i] : (canReuseValue ? currentValue.get(i) : undefined);
    }));
  }
  let newValueSrc = currentValueSrc;
  if (valueSrcFixes.length > 0 || !canReuseValue || (operatorCardinality || 0) < (currentOperatorCardinality || 0)) {
    newValueSrc = Immutable.List(Array.from({length: operatorCardinality || 0}, (_ignore: any, i: number) => {
      return valueSrcFixes[i] ?? (canReuseValue && currentValueSrc.get(i) || null);
    }));
  }
  let newValueType = currentValueType;
  if (valueTypeFixes.length > 0 || !canReuseValue || (operatorCardinality || 0) < (currentOperatorCardinality || 0)) {
    newValueType = Immutable.List(Array.from({length: operatorCardinality || 0}, (_ignore: any, i: number) => {
  ...
  if (rangeErrorObj) {
    ...
    const willFix = canFix && (operatorCardinality || 0) >= 2;
  ...
  const hasValueErrorChanged = currentValueError?.size !== valueErrors.length
    || valueErrors.filter((v: any, i: number) => (v != currentValueError.get(i))).length > 0;
