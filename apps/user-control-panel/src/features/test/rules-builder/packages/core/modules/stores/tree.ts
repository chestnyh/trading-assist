 ...
 */
const addNewGroup = (state: any, path: any, type: any, generatedId: any, properties: any, config: any, children: any = null, meta: any = {}) => {
  ...
  if (state !== origState) {
    if (!children && !isDefaultCase) {
      state = state.setIn(expandTreePath(groupPath, "children1"), OrderedMap());
 ...
 * @param {Immutable.Map} properties
 */
const removeGroup = (state: any, path: any, config: any) => {
  const targetItem = state.getIn(expandTreePath(path));
  if (!targetItem) {
    // incorrect path
    return state;
 ...
 * @param {Immutable.Map} properties
 */
const removeGroupChildren = (state: any, path: any, config: any) => {
  const targetItem = state.getIn(expandTreePath(path));
  if (!targetItem) {
    // incorrect path
    return state;
 ...
 * @param {Immutable.List} path
 */
const removeRule = (state: any, path: any, config: any) => {
  const targetItem = state.getIn(expandTreePath(path));
  if (!targetItem) {
    // incorrect path
    return state;
 ...
 * @param {bool} not
 */
const setNot = (state: any, path: any, not: any) => {
  const targetItem = state.getIn(expandTreePath(path));
  if (!targetItem) {
    // incorrect path
    return state;
 ...
 * @param {bool} lock
 */
const setLock = (state: any, path: any, lock: any) => {
  const targetItem = state.getIn(expandTreePath(path));
  if (!targetItem) {
    // incorrect path
    return state;
 ...
 */
const setConjunction = (state: any, path: any, conjunction: any) => {
  const targetItem = state.getIn(expandTreePath(path));
  if (!targetItem) {
    // incorrect path
    return state;
 ...
 * @param {object} config
 */
const addItem = (state: any, path: any, type: any, generatedId: any, properties: any, config: any, children: any = null) => {
  ...
  if (canAdd) {
    ...
    if (!hasChildren) {
      ...
      const newChildrenWithLast = OrderedMap({
        [id]: Map(item),
        [last.get("id")]: last
      });
 ...
 * @param {Immutable.List} path
 */
const removeItem = (state: any, path: any) => {
 ...
 * @param {Immutable.List} path
 */
const removeChildren = (state: any, path: any) => {
 ...
 * @param {object} config
 */
const moveItem = (state: any, fromPath: any, toPath: any, placement: any, config: any) => {
  ...
  if (isSameParent) {
    ...
    newTargetChildren = newTargetChildren.updateIn(expandTreeSubpath(sourceSubpathFromTarget, "children1"), (_oldChildren: any) => newSourceChildren);
  }
  ...
  if (isTargetInsideSource) {
    newSourceChildren = newSourceChildren.updateIn(expandTreeSubpath(targetSubpathFromSource, "children1"), (_oldChildren: any) => newTargetChildren);
    newSourceChildren = newSourceChildren.delete(from.get("id"));
  }

  if (!isSameParent && !isSourceInsideTarget)
    state = state.updateIn(expandTreePath(sourcePath, "children1"), (_oldChildren: any) => newSourceChildren);
  if (!isTargetInsideSource)
    state = state.updateIn(expandTreePath(targetPath, "children1"), (_oldChildren: any) => newTargetChildren);
 ...
 * @param {string} srcKey
 */
const setFieldSrc = (state: any, path: any, srcKey: any, config: any) => {
 ...
 */
const setFuncValue = (config: any, state: any, path: any, delta: any, parentFuncs: any, argKey: any, argValue: any, valueType: any, asyncListValues: any, _meta: any = {}) => {
  const currentRule = state.getIn(expandTreePath(path));
  if (!currentRule) {
    // incorrect path
    return state;
  ...
  for (const [funcK, argK] of (parentFuncs || []) as any[]) {
  ...
  while (funcsPath.length) {
    const popped = funcsPath.pop();
    if (!popped) break;
    const [funcK, argK, parentFV] = popped;
 ...
 */
const setField = (state: any, path: any, newField: any, config: any, asyncListValues: any, _meta: any = {}) => {
  const currentRule = state.getIn(expandTreePath(path));
  if (!currentRule) {
    // incorrect path
    return {state};
  ...
  if (wasRuleGroup && !isRuleGroup) {
    ...
    state = state.deleteIn(expandTreePath(path, "children1"));
    state = state.setIn(expandTreePath(path, "properties"), OrderedMap());
  }
  ...
  if (isRuleGroup) {
    ...
    state = state.setIn(expandTreePath(path, "children1"), OrderedMap());
    ...
    state = state.updateIn(expandTreePath(path, "properties"), (map: any) => map.withMutations((current: any) => {
      const {
 ...
 */
const setOperator = (state: any, path: any, newOperator: any, config: any) => {
  const currentRule = state.getIn(expandTreePath(path));
  if (!currentRule) {
    // incorrect path
    return state;
  ...
  state = state.updateIn(expandTreePath(path, "properties"), (map: any) => map.withMutations((current: any) => {
    ...
    const _currentValue = current.get("value", List());
    const _currentValueSrc = current.get("valueSrc", List());
 ...
 * @param {*} asyncListValues
 */
const setValue = (state: any, path: any, delta: any, value: any, valueType: any, config: any, asyncListValues: any, _meta: any = {}) => {
  const currentRule = state.getIn(expandTreePath(path));
  if (!currentRule) {
    // incorrect path
    return {state};
  ...
  const firstError = allErrors?.find((e: any) => !e.fixed && !e.ignore);
  ...
  const values = Array.from({length: operatorCardinality || 0}, (_, i) =>
    (i == delta ? value : state.getIn(expandTreePath(path, "properties", "value", i + "")) || null));
  const valueSrcs = Array.from({length: operatorCardinality || 0}, (_, i) =>
  ...
  if (showErrorMessage) {
    ...
    if (!lastValueErrorArr) {
      state = state
        .setIn(expandTreePath(path, "properties", "valueError"), List(new Array(operatorCardinality || 0)));
    }
    // set error at delta
    state = state.setIn(expandTreePath(path, "properties", "valueError", delta), validationError);
    // set range error
    if (operatorCardinality && operatorCardinality >= 2) {
 ...
 * @param {*} srcKey
 */
const setValueSrc = (state: any, path: any, delta: any, srcKey: any, config: any, _meta: any = {}) => {
  const currentRule = state.getIn(expandTreePath(path));
  if (!currentRule) {
    // incorrect path
    return state;
 ...
 * @param {*} value
 */
const setOperatorOption = (state: any, path: any, name: any, value: any) => {
  const currentRule = state.getIn(expandTreePath(path));
  if (!currentRule) {
    // incorrect path
    return state;
 ...
 * @param {Immutable.Map} state
 */
const checkEmptyGroups = (state: any, config: any) => {
  const {canLeaveEmptyGroup} = config.settings;
...
const initEmptyValueLists = (state: any, path: any, config: any, operatorCardinality: any) => {
  ...
  for (const k of ["value", "valueType", "valueError", "valueSrc"]) {
    if (!state.getIn(expandTreePath(path, "properties", k))) {
      state = state
        .setIn(expandTreePath(path, "properties", k), List(
...
const _addChildren1 = (config: any, item: any, children: any) => {
  if (children && Array.isArray(children)) {
    item.children1 = OrderedMap(
      children.reduce((map: any, it: any) => {
...
const getField = (state: any, path: any) => {
  const field = state.getIn(expandTreePath(path, "properties", "field")) || null;
  return field;
};
...
const getActionMeta = (action: any, state: any): any => {
  ...
  let meta: any = mapValues(omit(action, actionKeysToOmit), applyToJS);
 ...
 * @param {object} action
 */
export default (initialConfig: any, tree: any, getMemoizedTree: any, setLastTree: any, getLastConfig: any) => {
  const initTree = tree;
  const emptyState = {
    tree: initTree, 
    ...emptyDrag
  ...
  return (state: any = emptyState, action: any) => {
    const config = getLastConfig?.() ?? action?.config ?? initialConfig;
    const unset: any = {__lastAction: undefined};
    let set: any = {};
