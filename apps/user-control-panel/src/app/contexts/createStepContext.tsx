...
function pickFirstFieldErrors<T extends Record<string, unknown>>(issues: z.ZodIssue[]): FieldErrors<T> {
...
export function createStepContext<T extends Record<string, unknown>>(cfg: {
    ...
    function Provider({ children }: { children: React.ReactNode }) {
        const [state, dispatch] = useReducer<typeof reducer, undefined>(reducer, undefined, initState);
        ...
        useEffect(() => {
            ...
            try {
                window.localStorage.setItem(cfg.lsKey, JSON.stringify(stepData));
            } catch {
                // no-op
            }
        ...
        const value = useMemo<ContextValue>(() => {
            return {
                ...
                reset: () => {
                    ...
                    if (typeof window !== "undefined") window.localStorage.removeItem(cfg.lsKey);
                },
