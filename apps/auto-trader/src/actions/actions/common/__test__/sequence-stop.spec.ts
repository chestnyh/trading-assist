...
describe('sequence with stop_sequence', () => {
    ...
    it('should stop parent sequence when stop_sequence is called in nested sequence', async () => {
        const executed: string[] = [];

        // Mock debug action to track execution
        ...
        const config = {
            do: [
                {
                    type: 'debug',
                    arguments: { message: 'Step 1 - before error' }
                },
                {
                    type: 'if_then',
                    arguments: {
                ...
                {
                    type: 'debug',
                    arguments: { message: 'We reached here (SHOULD NOT SEE THIS!)' }
                }
            ]
        };
        ...
        expect(executed).toContain('debug: Step 1 - before error');
        expect(executed).toContain('debug: Error Happened');

        // Should NOT see the action after stop_sequence
        expect(executed).not.toContain('debug: We reached here (SHOULD NOT SEE THIS!)');
    });
